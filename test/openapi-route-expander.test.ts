...
describe('resolveRefsInRoutes', () => {
  it('throw error when input path is not absolute', () => {
  ...
  it('throw error when output path is not absolute', () => {
  ...
  it('throw error when input path does not exist', () => {
    expect(() => resolveRefsInRoutes(`${__dirname}/../example/input/openapi-1.yaml`, `${__dirname}/../out/openapi.yaml`))
      .toThrow(Error(`Path doesn't exists. "${__dirname}/../example/input/openapi-1.yaml"`));
  });
  it('writes the output of the file.', () => {
